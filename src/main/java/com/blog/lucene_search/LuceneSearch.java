package com.blog.lucene_search;
...
public class LuceneSearch {
	...
	public void updateIndex(Blog blog)throws Exception{
		...
		doc.add(new StringField("releaseDate",DateUtil.formatDate(new Date(), "yyyy-MM-dd"),Field.Store.YES));
		doc.add(new TextField("content",blog.getContentNoTag(),Field.Store.YES));
		iwriter.updateDocument(new Term("id", String.valueOf(blog.getId())), doc);//Term用于表示文档，由一个字段名和一个出现在文本中的值组成
		iwriter.close();
	}
	...
	public List<Blog> search(String query) throws Exception{
		...
		for(ScoreDoc scoreDoc:hits){
			...
			String content=StringEscapeUtils.escapeHtml(doc.get("content"));//转移文本中的Html标签
			if(title!=null){
